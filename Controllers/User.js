...
export const Login = async (req, res, next) =>{
  try{
    ...
    if(!isUser) return res.status(404).json('Sorry, user not found')

    const isMatched = await bcrypt.compare(password, isUser.password)
    if(!isMatched) return res.status(401).json('The password is wrong')

    const token = jwt.sign({id: isUser._id}, process.env.SECRET_KEY)

    if(isUser){
      //in order don't show password ib object in postman
